    ...
    <ul class="mr3 mt3 list-reset inline-block flex justify-end">
        ...
        <?php foreach($menu as $menuitem): ?>
            ...
            foreach($menuitem->children as $child) :
                if($child->menu_id == get_the_ID()) :
                    $highlight_parent = true;
                endif;
            endforeach;?>

            <li class=" relative <?=(!empty($menuitem->children))? "has-dropdown" : ""; ?>">
                <a href="<?=$menuitem->url?>" class="border-none text-center block bold px2 xl-px3 lg-h4 xl-h3 white hover-brand-primary <?=($menuitem->ID  == get_the_ID() || $highlight_parent)? "active":""?>">
