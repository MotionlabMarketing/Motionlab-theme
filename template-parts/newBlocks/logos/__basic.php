        ...
        <div class="col-12">
            ...
            <?php if (!empty($block['logos'])): ?>
                ...
                <div class="item-slider px5" data-slick="logo-slider-partners">&nbsp;
                    ...
                    <?php foreach ((array)$block['logos'] as $logo): $link = get_field("link", $logo['ID']);?>

                        <div class="col col-grid-5 flex items-center justify-center px5" data-mh="partners-slider">
                            
                            <?php if(!empty($link['url'])): ?><a href="<?=$link['url']?>" <?=($link['title'])? 'title="'.$link['title'].'"':""?> <?=($link['target'])? 'target="'.$link['target'].'"':""?>><?php endif; ?>
